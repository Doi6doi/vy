...
type
   ...
   TVServerIntf = class( TVIntf )
   protected
      FKnown: Boolean;
   public
      constructor Create;
      property Known: Boolean read FKnown write FKnown;
   end;
   ...
   public
      ...
      function FindFunc( Intf: TVIntf; Name: Utf8String; Check: Boolean ): TVFunc;
...
function TVSys.FindFunc( Intf: TVIntf; Name: Utf8String; Check: Boolean ): TVFunc;
var
   SI: TServerIntf;
begin
   Result := Intf.Func( Name, false );
   if (nil = Result) and (Intf is TServerIntf) then begin
      SI := TServerIntf(Intf);
      if not SI.Known then
...
begin
   if nil = FObjects then begin
      ...
      FObjects := FindFunc( II, VC.Objects, true );
   end;
   Result := FObjects;
end;
